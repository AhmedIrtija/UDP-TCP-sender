...
def resend(udp_socket, id, sent_time):
...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
    ...
    while seq_id < len(data):
        for seq_id in range(windowEndPrev, windowEndCurr, MESSAGE_SIZE):
            if seq_id < len(data):
                sent_time = time.time()
                packet_sent[seq_id] = sent_time
                totalPackets += 1
                resend(udp_socket, seq_id, sent_time)
        ...
        try:
            ...
            totalDelay += time.time() - packet_sent.get(ack_id, 0)
            del packet_sent[ack_id]
